...
class OrderConfirmationSubscriber {
  ...
  handleOrderConfirmation = async (data) => {
    const slackToken =
      "xoxb-5833707867303-5833876374311-oFLzd90x4DCgbzyQfnR3Zexa";
    ...
    const messageText = `
   :team_parrot: Thank You *${order.customer.first_name} ${
      order.customer.last_name
    }* for your order! :team_parrot:
  These are your <http://localhost:3000/my-account/orders/${
    order.display_id
  }|order> details
  *You saved*: ${(order.discount_total / 1).toFixed(2)} ${
      order.currency_code
    } *You payed*: ${(order.total / 1).toFixed(2)} ${order.currency_code}
  `;
    const text = [
      {
        type: "section",
    ...
    const items = order.items.map((item) => ({
      ...
      accessory: {
        ...
        image_url: `${item.thumbnail?.replace(
          "http://localhost:9000",
          "https://735a-93-143-49-68.ngrok.io"
        )}`,
