...
import { Region, Cart, AddressPayload } from '@medusajs/medusa';
import {
...
interface StoreContext {
  ...
  changeCartAddress: (address: AddressPayload, billing?: boolean) => void;
  addDiscount: (discount: string) => void;
  deleteDiscount: () => void;
  changeCartEmail: (email: string) => void;
}
...
export const StoreProvider = ({ children }: StoreProps) => {
  ...
  const addShipping = (option_id: string) => {
  ...
  const changeCartEmail = (newemail: string) => {
  ...
  const changeCartAddress = (address: AddressPayload, billing?: Boolean) => {
    if (!billing) {
      updateCart.mutate(
        {
          shipping_address: {
        ...
        {
          onSuccess: ({ cart }) => {
            console.log('cart shipping', cart.shipping_address);
            setCart(cart);
            storeCart(cart.id);
            timedOpen();
          },
      ...
      updateCart.mutate(
        ...
        {
          onSuccess: ({ cart }) => {
            console.log('cart biling', cart.billing_address);
            setCart(cart);
            storeCart(cart.id);
            timedOpen();
          },
